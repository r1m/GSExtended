...
function AutoCompletePopup(field, characters, fetchFunction, clickHandler) {
	...
	// name.
	if (!this.clickHandler) this.clickHandler = function(replacement) {
		...
		if (selection.length == 0) {
			...
			if (nameStart >= 0) {
				...
				var p = nameStart + replacement.length + 1;
	...
	this.field.keyup(function(e) {
		switch (e.which) {
			...
			default:
				...
				else ac.fetchNewContent($(this).val(),0);
				break;
		}
	});
	...
	this.update = function(results) {
		...
		if (results.length) {
			...
			var searchString = ac.value.replace(/ /g, "\xA0").replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
			...
			for (var i in results) {
				...
				var name = $("<div/>").text(results[i].text).html();
